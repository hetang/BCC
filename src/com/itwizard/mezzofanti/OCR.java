...
public final class OCR 
{
	...
	public static String m_sOCRResult = null;
	...
	public final static class OCRConfig
	{
		...
		public boolean GetSettings(SharedPreferences prefs)
		{
			...
			/*try
			{
				OCR.mConfig.m_iMinOveralConfidence = Integer.parseInt(prefs.getString(PreferencesActivity.KEY_MIN_OVERALL_CONFIDENCE, "60"));
				OCR.mConfig.m_iMinWordConfidence = Integer.parseInt(prefs.getString(PreferencesActivity.KEY_MIN_WORD_CONFIDENCE, "50"));
				OCR.mConfig.SetImgDivisor(Integer.parseInt(prefs.getString(PreferencesActivity.KEY_SPEED_QUALITY, "2")));
				OCR.mConfig.m_bUseBWFilter = prefs.getBoolean(PreferencesActivity.KEY_USE_IMAGE_LIGHT_FILTER, false);
				OCR.mConfig.m_sLanguage = prefs.getString(PreferencesActivity.KEY_SET_OCR_LANGUAGE, "eng");
			}
			catch (Exception ex)
			{
	...
	private OCR()
	{
		...
		Log.v(TAG, "mConfig.m_asLanguages = " + mConfig.m_asLanguages);
	...
	public static void ReadAvailableLanguages()
	{
		...
		if(mConfig.m_asLanguages == null) {
			mConfig.m_asLanguages = new String[]{};
		}
	}
	...
	public boolean AddUserWord(String newword)
	{
		if (!isValidWord(newword))
		{
			...
			try 
			{
				writer = new BufferedWriter(new FileWriter(Mezzofanti.DATA_PATH + mConfig.GetLanguage() +".user-words",true));
	...
	{	
		...
		try
		{
			tif = GetBytesFromFile(new File(Mezzofanti.RESULTS_PATH + "tessinput.tif"), pic_width, pic_height, 1);
		}
		catch (Exception ex)
		{
	...
	public String ImgOCRAndFilter(int[] bw_img, int width, int height, boolean bHorizontalDisp, boolean bLineMode)
	{
		...
		try {
			...
			if (mConfig.m_iMinWordConfidence > 0 && !m_bLineMode)
			{
				...
				for (int i=0; i<m_asWords.length; i++)
				{
					...
					Log.v(TAG,"m_asWords[i].m_sBody = " + m_asWords[i].m_sBody + " m_asWords[i].m_iConfidence = " + m_asWords[i].m_iConfidence + " mConfig.m_iMinWordConfidence = " + mConfig.m_iMinWordConfidence);
					//if (m_asWords[i].m_iConfidence > mConfig.m_iMinWordConfidence)
					if (m_asWords[i].m_iConfidence > 20)
					{   
		...
		if (!m_bLineMode)
		{
			...
			for (int i=0; i<m_asWords.length; i++)
			{
				...
				{
					m_ssOCRResult.setSpan(new StyleSpan(Typeface.ITALIC), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
					Log.v(TAG, "ss: "+start+"-"+end);
				}
				start = end;
			}
		}
		m_sOCRResult = text_ret;
		return text_ret;
	}
